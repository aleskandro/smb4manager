...
@login_required
def index():
    model = UserModel2(session['smb4'][0]['username'],
        session['smb4'][0]['password'])
    users = model.ListUsers()
    return render_template('users/index.html',
        users=users, utils=session['utils'])
...
def users_add():
    if ((request.method == "POST") and (request.form['submit'] == 'users_add')):
        ...
        return_error = 0
        model = UserModel(session['smb4'][0]['username'],
                session['smb4'][0]['password'])
        ...
        newmodel = UserModel2(session['smb4'][0]['username'],
            session['smb4'][0]['password'])
